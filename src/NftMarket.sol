...
error NotOwner();
    ...
    function createMessageHash(
        ...
        return keccak256(abi.encodePacked(nftAddress, tokenId, price, deadline));
    }
    ...
    {
        require(deadline > 1 hours, "Deadline must be greater than 1 hour in the future");
        if (price <= 0) {
    ...
    function buyItem(uint256 orderId, bytes32 messageHash)
        external
        payable
        isListed(orderId) isExpired(idToListing[orderId].deadline)
    {
        ...
        IERC721(idToListing[orderId].nftAddress).safeTransferFrom(listedItem.seller, msg.sender, orderId);
    }
